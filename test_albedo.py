...
parser.add_argument('--gradX_model')
parser.add_argument('--gradY_model')
...
recon_size = 960 # if training 1216 if test 960
recon_patch_num = 15 # if training 19 if test 15

save_dir = 'multitask_albedo_denoise_finetune_6_11'
...
def denoise(models, dataloader, device, mode, save_dir, do_vis_feat, debug=False):
  ...
  # gradXNet = models['gradX']
  # gradYNet = models['gradY']

  with torch.no_grad():
    criterion = nn.L1Loss()
    relL2 = RelativeMSE()
    L2 = nn.MSELoss()
    lossDiff, lossSpec, lossFinal, relL2Final, lossAlbedo, lossGradX, lossGradY= 0,0,0,0,0,0,0
    scenelossDiff, scenelossSpec, scenelossFinal, scenelossAlbedo, scenelossGradX, scenelossGradY, scenerelL2Final= 0,0,0,0,0, 0, 0
    image_idx = 0
    input_image = torch.zeros((3, recon_size, recon_size)).to(device)
    gt_image = torch.zeros((3, recon_size, recon_size)).to(device)
    output_image = torch.zeros((3, recon_size, recon_size)).to(device)
    albedo_image = torch.zeros((3, recon_size, recon_size)).to(device)
    gradX_image = torch.zeros((3, recon_size, recon_size)).to(device)
    gradY_image = torch.zeros((3, recon_size, recon_size)).to(device)
    gt_albedo_image = torch.zeros((3, recon_size, recon_size)).to(device)
    gt_gradX_image = torch.zeros((3, recon_size, recon_size)).to(device)
    gt_gradY_image = torch.zeros((3, recon_size, recon_size)).to(device)

    # Error calculation
    error_map = torch.zeros((3, recon_size, recon_size)).to(device)
    ...
    for data in tqdm(dataloader, leave=False, ncols=70):
      ...
      background_mask_r, background_mask_g, background_mask_b = (gt_albedo[:,0,:,:]<0.001), (gt_albedo[:,1,:,:]<0.001), (gt_albedo[:,2,:,:]<0.001)
      # print(background_mask_r.shape)
      ...
      #         background_mask[i, 2, j, k] = False
			...
			# g-buffer estimation
      # gradX = crop_like(data['kpcn_diffuse_in'][:,4:7,:,:], outputDiff).to(device)
      # gradY = crop_like(data['kpcn_diffuse_in'][:,7:10,:,:], outputDiff).to(device)
      enc_in = ToneMapBatch(outputFinal).to(device)
      # print(gt_albedo.shape)
      # gt_albedo[background_mask] += enc_in[background_mask]
      enc_in[background_mask] = 0
      # enc = encodeNet(outputFinal)
      # enc = encodeNet(enc_in)
      ...
      # lossGradY += criterion(outputGradX, gradY.unsqueeze(1)).item()
      # lossGradX += criterion(outputGradY, gradX).item()
      ...
      # gradX_image[:, x*64:x*64+64, y*64:y*64+64] = outputGradX[0, :, 16:80, 16:80]
      # gradY_image[:, x*64:x*64+64, y*64:y*64+64] = outputGradY[0, :, 16:80, 16:80]
      gt_albedo_image[:, x*64:x*64+64, y*64:y*64+64] = gt_albedo[0, :, 16:80, 16:80]
      # gt_gradX_image[:, x*64:x*64+64, y*64:y*64+64] = gradX[0, :, 16:80, 16:80]
      # gt_gradY_image[:, x*64:x*64+64, y*64:y*64+64] = gradY[0, :, 16:80, 16:80]
      ...
      if 'simple_feat' in mode:
        ...
        foutDiff = open(save_dir + '/test{}/attns/patch_{}_diff_attn.csv'.format(image_idx, recon_patch_num*x+y), 'w')
        foutSpec = open(save_dir + '/test{}/attns/patch_{}_spec_attn.csv'.format(image_idx, recon_patch_num*x+y), 'w')
        for fi in range(0, 9):
          pass
        ...
        if not os.path.exists(save_dir + '/test{}/attns'.format(image_idx)):
            os.makedirs(save_dir + '/test{}/attns'.format(image_idx))
        fout = open(save_dir + '/test{}/attns/patch_{}_attn.csv'.format(image_idx, recon_patch_num*x+y), 'w')
        for fi in range(9):
          for fj in range(21):
      ...
      y += 1
      if x < recon_patch_num and y>=recon_patch_num:
        x += 1
        y = 0

      if x >= recon_patch_num:
        ...
        save_image(gt_albedo_image, save_dir + '/test{}/gt_albedo.png'.format(image_idx))
        # save_image(gradX_image, save_dir + '/test{}/gradX.png'.format(image_idx))
        # save_image(gradY_image, save_dir + '/test{}/gradY.png'.format(image_idx))
          
        # losses
        scenelossDiff, scenelossSpec, scenelossFinal, scenerelL2Final = scenelossDiff/(recon_patch_num**2), scenelossSpec/(recon_patch_num**2), scenelossFinal/(recon_patch_num**2), scenerelL2Final/(recon_patch_num**2)
        scenelossFinal = criterion(output_image, gt_image)
        scenerelL2Final = relL2(output_image, gt_image)
        scenelossAlbedo = criterion(albedo_image, gt_albedo_image)
        # scenelossGradX = criterion(gradX_image, gt_gradX_image)
        # scenelossGradY = criterion(gradY_image, gt_gradY_image)
        print()
        ...
        print('test {} lossAlbedo : {}'.format(image_idx, scenerelL2Final))
        # print('test {} lossGradX : {}'.format(image_idx, scenerelL2Final))
        # print('test {} lossGradY : {}'.format(image_idx, scenerelL2Final))
        ...
        writer.add_scalar('test lossAlbedo',  scenelossAlbedo if lossFinal != float('inf') else 1e+35, image_idx)
        # writer.add_scalar('test lossGradX',  scenelossGradX if lossFinal != float('inf') else 1e+35, image_idx)
        # writer.add_scalar('test lossGradY',  scenelossGradY if lossFinal != float('inf') else 1e+35, image_idx)
  ...
  return lossDiff/len(dataloader), lossSpec/len(dataloader), lossFinal/len(dataloader), relL2Final/len(dataloader), lossAlbedo/len(dataloader), lossGradY/len(dataloader), lossGradX/len(dataloader)
...
def test_model(models, device, data_dir, mode, args):
  ...
  _, _, totalL1, totalrelL2, totalAlbedo, totalGradX, totalGradY = denoise(models, dataloader, device, mode, args.save_dir, args.do_vis_feat)
  print('TEST L1 LOSS is {}'.format(totalL1))
  print('TEST L2 LOSS is {}'.format(totalrelL2))
  print('TEST ALBEDO LOSS is {}'.format(totalAlbedo))
  print('TEST DEPTH LOSS is {}'.format(totalGradX))
  print('TEST NORMAL LOSS is {}'.format(totalGradY))
...
def main():
  ...
  # albedoNet, gradXNet, gradYNet = decoderUnet(mode=0).to(args.device), decoderUnet(mode=0).to(args.device), decoderUnet(mode=0).to(args.device)
  # albedoNet = decoderUnet(mode=0).to(args.device)

  albedoNet = make_recon_net(args.num_layers, 3, 100, 3, 'dpcn').to(args.device)
  ...
  checkpointX = torch.load(args.gradX_model)
  checkpointY = torch.load(args.gradY_model)
  ...
  # gradXNet.load_state_dict(checkpointX['model_state_dict'])
  # gradYNet.load_state_dict(checkpointY['model_state_dict'])
